...
function[newPose,y,muParticleNew,covParticleNew,newParticleSet,meas_ind,newfeature,w_new]...
        =fastSLAM(oldPose,u,MotionNoise,MeasurementNoise,map,particleSet,muFeatOld,covFeatOld,rmax,thmax,timeInterval,totalParticles,newfeature,w)
    ...
    for particle=1:totalParticles
       ...
       for j=1:length(meas_ind)
           ...
           [H_feat,pred_obs]=predict_features(muFeatOld(:,i,particle),particleSetP(:,particle));
           ...
           Qw(2*(j-1)+1:2*j,2*(j-1)+1:2*j) = H_feat*covFeatPred(:,:,i,particle)*H_feat'+R;
       end

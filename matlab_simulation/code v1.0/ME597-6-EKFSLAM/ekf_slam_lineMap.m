...
% Control inputs
% u = [ones(2,floor(length(T)./4)) -ones(2,floor(length(T)./4)),...
%     ones(2,floor(length(T)./4)), -ones(2,floor(length(T)./4))];
% if length(u)<length(T)
%    u=[u,zeros(2,length(T)-length(u))]; 
% end
turns=6;    % number of 180 deg turns to make
u=ones(2,length(T));
u(2,:)=0;
for n=1:turns

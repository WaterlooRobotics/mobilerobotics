...
if (mode == 1)
    ...
    invmod = inverse_scanner_window(w_M, w_N, x(1), x(2), x_w, y_w, x(3), phi_m, r_m, r_max, alpha, beta, 0.7, 0.3);
    
    % Calculate updated log odds
    og(w_Mi:w_Mf, w_Ni:w_Nf) = og(w_Mi:w_Mf, w_Ni:w_Nf) + log(invmod./(1-invmod))-L0(w_Mi:w_Mf, w_Ni:w_Nf);
    imml(w_Mi:w_Mf, w_Ni:w_Nf) = imml(w_Mi:w_Mf, w_Ni:w_Nf) +log(invmod./(1-invmod))-L0(w_Mi:w_Mf, w_Ni:w_Nf);
else
    ...
    for i = 1:length(phi_m)
        % Get inverse measurement model
        invmod = inversescannerbres(M, N, x(1), x(2), phi_m(i)+x(3), r_m(i), r_max);

%% Rapidly Expanding Random Tree example - Direct RRT
clear; clc;
...
%% Direct RRT, created until solution found
tic;
done = 0;
milestones = [x0 0];
nM = 1;
...
while ((~done) && (t < max_iters))
    ...
    while (~goal_found)
        ...
        cur_goal = (1-growth_factor)*x0(1:2) + growth_factor * xR.*rand(1,2);
    ...
    cur_line = [milestones(curstone,1:2); cur_goal];
    %plot(cur_line(:,1), cur_line(:,2),'k','LineWidth',2);
    
    % Get new control input and trajectory
    newstone = 0;
    s = 0;
    dist2curgoal = norm(cur_goal - milestones(curstone,1:2));
    while ((s < 10))
        s=s+1;
        ...
        if (sum(keep)==length(samples(:,1)))
            newdist = norm(samples(end,1:2)-cur_goal);
            if (newdist < dist2curgoal)
                newstone = 1;
                dist2curgoal = newdist;
                newsamples = samples;
            end
        end
    end
    if(newstone)
        milestones = [milestones; newsamples(end,:) curstone];
        nM = nM+1;
        plot(newsamples(:,1),newsamples(:,2),'m');
